...
mkPolicy pkh deadline () ctx =  traceIfFalse "invalid signature!" validSig    &&
                                traceIfFalse "deadline reached" beforeDeadline
    where
        info :: TxInfo
        info = scriptContextTxInfo ctx

        validSig :: Bool
        validSig = txSignedBy info $ unPaymentPubKeyHash $ pkh

        beforeDeadline :: Bool
        beforeDeadline = after deadline $ txInfoValidRange info
