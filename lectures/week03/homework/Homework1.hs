...
mkValidator dat () ctx 
    | deadlineReached = traceIfFalse "invalid beneficiary" $ signedByBeneficiary (beneficiary2 dat)
    | otherwise = traceIfFalse "invalid beneficiary" $ signedByBeneficiary (beneficiary1 dat)
  where
    info :: TxInfo
    info = scriptContextTxInfo ctx

    signedByBeneficiary :: PaymentPubKeyHash -> Bool
    signedByBeneficiary ppkh = txSignedBy info $ unPaymentPubKeyHash ppkh
