...
transition game s r = case (stateValue s, stateData s, r) of
    ...
    (v, GameDatum _ (Just c2), Reveal _ c1)
        | lovelaces v == (2 * gStake game) && (c1 /= c2)  -> Just ( Constraints.mustBeSignedBy (gFirst game)  <>
                                                      Constraints.mustValidateIn (to $ gRevealDeadline game)
                                                     , State Finished mempty
                                                     )
    (v, GameDatum bs (Just c2), Reveal _ c1)
        | lovelaces v == (2 * gStake game) && (c1 == c2) -> Just ( Constraints.mustBeSignedBy (gFirst game)  <>
                                                       Constraints.mustValidateIn (to $ gRevealDeadline game)
                                                     , State (GameDatum bs $ Just c2) (lovelaceValueOf $ gStake game)
                                                     )
    ...
    (v, GameDatum _ (Just _), ClaimSecond) -> Just ( Constraints.mustBeSignedBy (gSecond game)  <>
                                                       Constraints.mustValidateIn (from $ 1 + gRevealDeadline game)
                                                     , State Finished mempty
                                                     )
    _                                               -> Nothing

    --- lovelaces v == (2 * gStake game) || lovelaces v == gStake game
...
check bsRock' bsPaper' bsScissors' (GameDatum bs (Just c2)) (Reveal nonce c1) _ = 
        (checkByteStringMatches nonce c1 bs) --- && (c2 == c1 || (beats c1 c2))
    where
        checkByteStringMatches :: BuiltinByteString -> GameChoice -> BuiltinByteString -> Bool
        checkByteStringMatches n c1 bs = sha2_256 (n `appendByteString` case c1 of
          Rock     -> bsRock'
          Paper    -> bsPaper'
          Scissors -> bsScissors') == bs
    ...
    case m of
        ...
        Just (o, _) -> case tyTxOutData $ ocsTxOut o of
            ...
            GameDatum _ (Just c') | c' == c -> do
                ...
                logInfo @String "first player revealed and took his stake"
        ...
        Just (o, _) -> case tyTxOutData $ ocsTxOut o of
            GameDatum _ Nothing -> do
                ...
                case m' of
                    ...
                    Just _  -> do
                        logInfo @String "first player didn't reveal or was a Draw"
                        void $ mapError' $ runStep client ClaimSecond
                        logInfo @String "second player won or was a Draw"
